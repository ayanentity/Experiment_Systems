...
import { PostPracticeTestQuestionRepository } from "@/src/infrastructure/repositories/QuestionRepository";
...
export default function PostPracticeTestPage() {
  ...
  const courseName = "事後テスト";

  // テスト問題を取得
  const repository = new PostPracticeTestQuestionRepository();
      ...
      <main className="flex min-h-screen w-full max-w-4xl flex-col items-center justify-center gap-8 py-16 px-8 bg-white dark:bg-black">
        ...
        <div className="w-full max-w-2xl p-4 bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-200 dark:border-yellow-800 rounded-lg">
          <p className="text-sm text-yellow-800 dark:text-yellow-200 text-center">
            ⚠️ 事後テスト：結果は後でまとめて表示されます。集中して練習の成果を試しましょう。
          </p>
        </div>
